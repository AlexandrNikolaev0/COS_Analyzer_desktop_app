...
class Ui_ConnectWindow(object):
    ...
    def addTask(self, task):
        ...
        self.ui.BinBtn = QtWidgets.QPushButton(self.ui.tableWidget)
        self.ui.BinBtn.setMinimumSize(QtCore.QSize(32, 32))
        self.ui.BinBtn.setMaximumSize(QtCore.QSize(32, 32))
        self.ui.BinBtn.setStyleSheet("QPushButton{"
                                     "min-width:32px;"
                                     "max-width:32px;"
                                     "min-height:32px;"
                                     "max-height:32px;"
        ...
        self.ui.BinBtn.setIcon(icon)
        self.ui.BinBtn.setIconSize(QtCore.QSize(20, 20))
        ...
        self.ui.BinBtn.clicked.connect(lambda: self.binBtnHandler(self.widget))
    ...
    def binBtnHandler(self, binButton):
        pos = self.ui.tableWidget.currentRow()
        print(pos)
        pos = self.ui.tableWidget.indexAt(binButton.pos()).row()
        print(pos)
            ...
            try:
                ...
                if pos >= 0:  # добавить проверку на павильность задачи когда уточнятся диапазоны данных
                    ...
                    if str(self.ui.tableWidget.cellWidget(pos, 1).text()) in ["Ожидание", "Прервано"]:
                        ...
                        label3.setText(str(datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S")))
                        ...
                        self.taskTable[pos].totalTime = datetime.datetime.now()
                    self.isUpdatable = True
    ...
    def stopBtnHandler(self):
        ...
        try:
            ...
            if pos >= 0 and str(self.ui.tableWidget.cellWidget(pos, 1).text()) == "В процессе":
                ...
                label4.setText(str(datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S")))
                ...
                label5.setText(str(datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S")))
                ...
                self.ui.tableWidget.cellWidget(pos, 1).setStyleSheet(style1)
                if (type(self.taskTable[pos]) is sampleTask):
                    self.ui.tableWidget_hist.setItem(self.ui.tableWidget_hist.rowCount() - 1, 0,
                                                     QtWidgets.QTableWidgetItem(str(self.taskTable[pos].stopDate)))
                    self.ui.tableWidget_hist.setItem(self.ui.tableWidget_hist.rowCount() - 1, 1,
                                                     QtWidgets.QTableWidgetItem(str(self.taskTable[pos].description)))
                    self.ui.tableWidget_hist.setItem(self.ui.tableWidget_hist.rowCount() - 1, 4,
                                                     QtWidgets.QTableWidgetItem(str(self.taskTable[pos].totalTime)))
